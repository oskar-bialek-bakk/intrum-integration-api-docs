...
W trybie API-2-API import przechodzi przez 4 etapy. Klient API steruje etapem Adding i opcjonalnie etapem Validation (jeśli waliduje dane u siebie); API odpowiada za pozostałe.
...
!!! note "Stage 2 Transformation"
    Wartość `Stage=2 (Transformation)` jest celowo pominięta w tej dokumentacji. W trybie API-2-API nie jest używana, bo komunikaty są już dostarczane przez Klienta API w docelowym formacie.
